...
local function CreateQuickSignupRoleButton(parent, roleKey, tooltipText)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine(L["These buttons control the roles Oak uses for Quick Sign Up, the roles Oak preselects in the Blizzard popup, and your Dungeon Finder / Raid Finder role selectors."], 0.9, 0.9, 0.9, true)
        GameTooltip:Show()
    end)
...
quickSignupState.roleButtons.TANK = CreateQuickSignupRoleButton(quickSignupBar, "TANK", L["Use Tank for Quick Sign Up and preselect Tank in the Blizzard popup"])
quickSignupState.roleButtons.HEALER = CreateQuickSignupRoleButton(quickSignupBar, "HEALER", L["Use Healer for Quick Sign Up and preselect Healer in the Blizzard popup"])
quickSignupState.roleButtons.DAMAGER = CreateQuickSignupRoleButton(quickSignupBar, "DAMAGER", L["Use Damage for Quick Sign Up and preselect Damage in the Blizzard popup"])
...
signupLimitNote:SetText(L["Applied to 0/5 Groups"])
...
cancelOldestLabel:SetText(L["Cancel Oldest"])
...
cancelOldestBtn:SetScript("OnEnter", function(self)
    GameTooltip:SetOwner(self, "ANCHOR_TOP")
    GameTooltip:SetText(L["Cancel Oldest Application"], 1, 0.5, 0.5)
    GameTooltip:AddLine(L["Cancels your oldest pending group application to free up a signup slot."], 0.9, 0.9, 0.9, true)
    GameTooltip:Show()
end)
...
function addonTable.UpdateSignupLimitDisplay()
    ...
    if atMax then
        if not cooldownTimerRunning then   -- don't overwrite cooldown label text when it's active
            signupLimitNote:SetText(string.format(L["Applied to %d/%d Groups"], tonumber(numActive) or maxApps, maxApps))
            signupLimitNote:SetTextColor(1, 0.35, 0.35)
            signupLimitNote:Show()
        end
    else
        signupLimitNote:SetText(string.format(L["Applied to %d/%d Groups"], tonumber(numActive) or 0, maxApps))
        signupLimitNote:SetTextColor(0.85, 0.85, 0.85)
        if not cooldownTimerRunning then
            signupLimitNote:Show()
        end
...
quickSignupToggleBox:SetScript("OnEnter", function(self)
    ...
    GameTooltip:AddLine(L["When enabled, clicking Apply will immediately sign up using the roles shown in this bar."], 1, 1, 1, true)
    GameTooltip:AddLine(L["Hold Shift while clicking Apply to open the normal Blizzard popup instead."], 0.8, 0.8, 0.8, true)
    GameTooltip:Show()
end)
